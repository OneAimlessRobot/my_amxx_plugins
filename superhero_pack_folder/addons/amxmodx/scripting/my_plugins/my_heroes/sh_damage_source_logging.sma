...
sh_log_damage_source_primitive(hero_id=-1,short_name[SAFE_BUFFER_SIZE+1]="",long_name[SAFE_BUFFER_SIZE+1]="",is_a_melee=1){
        ...
        new short_name_arr[SAFE_BUFFER_SIZE]
        new long_name_arr[SAFE_BUFFER_SIZE]

        sh_damage_source_hero_ids[wpn_id]=-1
        sh_damage_source_is_a_melee[wpn_id]=generic_dmg_source_is_melee:is_a_melee;
...
public _sh_log_custom_damage_source(iPlugin,iParams){
    ...
    if((hero_id<0 )|| (hero_id >=SH_MAXHEROS)){
        ...
        get_array(2,short_name_arr,SAFE_BUFFER_SIZE)
        get_array(3,long_name_arr,SAFE_BUFFER_SIZE)
        is_a_melee=get_param(4)
    }
    sh_log_damage_source_primitive(hero_id=-1,short_name[SAFE_BUFFER_SIZE+1]="",long_name[SAFE_BUFFER_SIZE+1]="",is_a_melee=1)
    new wpn_id=custom_weapon_add(long_name_arr, is_a_melee, short_name_arr)
    if(wpn_id>0){
