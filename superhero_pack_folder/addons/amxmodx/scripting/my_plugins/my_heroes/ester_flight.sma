...
public Ester_DamageReflect(id, idinflictor, attacker, Float:damage, damagebits)
{
	...
	if(Get_BitVar(g_flying_mask,id)&&
		sh_user_has_hero(id,ester_get_hero_id())&&
		Get_BitVar(g_ester_is_reborn_mode_mask,id)&&
...
public ester_respawn(parm[])
{
	...
	sh_chat_message(id,ester_get_hero_id(), "%d attempts to change left",
...
public BlowUp(param[1],id)
{
	...
	if(!sh_is_freezetime()&&is_user_alive(id)&&sh_user_has_hero(id,ester_get_hero_id())&&(param[0]<ESTER_REBORN_EXPLOSION_NUMBER)){
		param[0]++
		explosion(ester_get_hero_id(),id,
...
explosion(ester_get_hero_id(),pToucher,150.0,(0.2*killer_speed),
