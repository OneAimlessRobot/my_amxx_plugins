...
public plugin_init()
{
	...
	create_cvar("arcticPredator_level", "20" )
	create_cvar("arcticPredator_invisible", "20")
	create_cvar("arcticPredator_explode_radius", "500" )
	create_cvar("arcticPredator_explode_maxdamage", "190" )
	create_cvar("arcticpredator_discspeed", "1200" )
	create_cvar("arcticpredator_discreturntime", "1.0")
	create_cvar("arcticpredator_discdmg", "60" )
	create_cvar("arcticPredator_huntmode", "10" )
	create_cvar("arcticPredator_cooldown", "30" )
...
{
	engfunc(EngFunc_PrecacheModel,"models/shmod/predgun/preddisc.mdl")
...
public StartNVG(id)
{
	...
	if (g_huntTimer[id] == 0 ) set_task(0.1,"RunNVG",id+TASKID_NVG,"",0,"b")
	if (g_huntTimer[id] > 0 ) set_task(0.1,"RunRedNVG",id+TASKID_NVG,"",0,"b")
}
//----------------------------------------------------------------------------------------------
public StopNVG(id)
{
	emit_sound(id,CHAN_ITEM,"items/nvg_off.wav",VOL_NORM,ATTN_NORM,0,PITCH_NORM)
	remove_task(id+TASKID_NVG)
...
public sh_client_death(id,attacker)
{
	...
	if ( killer[id] != id ) {
		...
		if(sh_get_user_has_hero(killer[id],gHeroID)&&killer[killer[id]]==id){
			...
			client_print(killer[id],print_chat,"[SH Predator] Revenge taken on %s. Ceasing tracking.", namea)
			killer[killer[id]]=0;
