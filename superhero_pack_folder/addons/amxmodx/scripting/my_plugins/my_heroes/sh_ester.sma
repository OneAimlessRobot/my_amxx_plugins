...
public plugin_init()
{
	...
	pcvar_gHeroLevel = register_cvar("ester_level", "8")
	pcvar_stun_time_at_it = register_cvar("ester_stuntime", "10")
	pcvar_stun_speed_at_it = register_cvar("ester_stunspeed", "100")
	pcvar_base_dmg_per_it = register_cvar("ester_damage", "3")
	pcvar_max_dmg = register_cvar("ester_max_dmg", "20")
	pcvar_dmg_inc_per_inc = register_cvar("ester_dmg_inc_per_inc", "5")
	pcvar_num_lvls_for_inc = register_cvar("ester_lvls_for_inc", "5")
	pcvar_times_per_map = register_cvar("ester_uses_per_map","1")
	pcvar_power_cost = register_cvar("ester_power_cost", "3")
	pcvar_pan_dmg = register_cvar("ester_pan_dmg", "1.5")
	pcvar_tmp_dmg_mult = register_cvar("ester_tmp_dmg_mult", "1.5")
	pcvar_min_damage_to_do = register_cvar("ester_min_damage_to_do", "1.5")
	pcvar_max_moralizing_xp = register_cvar("ester_max_moralizing_xp","9000") 
	pcvar_moralizing_tmp_xp_give = register_cvar("ester_moralizing_tmp_xp_give","4.0")
	pcvar_moralizing_pan_xp_give = register_cvar("ester_moralizing_pan_xp_give","16.0")
	pcvar_moralizing_headshot_xp_mult = register_cvar("ester_moralizing_headshot_xp_mult","4");
	pcvar_moralizing_tmp_xp_get_mult = register_cvar("ester_moralizing_tmp_xp_get_mult","5.0"); 
	pcvar_moralizing_pan_xp_get_mult = register_cvar("ester_moralizing_pan_xp_get_mult","25.0");
				...
				if(enemy_count>0){
					if ( (userArmor < cvar_val(num,pcvar_power_cost))) {
						if ( user_health< cvar_val(num,pcvar_power_cost) ) {
							...
							sh_extra_damage(i,i,1, dmg_source_name_short_neuroblast,HIT_HEAD,SH_DMG_KILL,_,_,_,_,
											SH_NEW_DMG_ENERGY_BLAST,
											neuroblast_wpn_id)
							continue;
						}
						sh_extra_damage(i,i,cvar_val(num,pcvar_power_cost), dmg_source_name_short_neuroblast,HIT_HEAD,_,_,_,_,_,
										SH_NEW_DMG_ENERGY_BLAST,
										neuroblast_wpn_id)
						user_health=get_user_health(i)
					}
...
public Ester_instant(x, id)
{
	...
	sh_extra_damage(x, id,gEsterDmg[id], dmg_source_name_short_neuroblast,HIT_HEAD,_,_,_,_,_,
									SH_NEW_DMG_ENERGY_BLAST,
									neuroblast_wpn_id)
		...
		if(!Get_BitVar(gUnloadingMask,id)&&gTimesLeft[id]){
			...
			if(Get_BitVar(gPedalIsFlooredMask,id)){
				...
				sh_extra_damage(attacker, id,
								...
								new_dmg_type_names[_:SH_NEW_DMG_SHOCK],hitpoint,_,_,_,_,_,
								SH_NEW_DMG_SHOCK,
				...
				if((sh_get_user_effect(id)<KILL)||(sh_get_user_effect(id)>BATH)){
					...
					if (extraDamage>0){
						...
						sh_extra_damage(id,attacker,floatround(extraDamage),
									(weapon==CSW_TMP)?dmg_source_name_short_moralizing_ray:dmg_source_name_short_adulting_pan,hitgroup,_,_,_,_,_,
									_,
