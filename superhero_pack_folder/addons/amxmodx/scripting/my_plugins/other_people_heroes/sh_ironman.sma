...
public plugin_init()
{
	...
	new pcvarLevel = create_cvar("ironman_level", "0")
	new pcvarArmor = create_cvar("ironman_armor", "100")
	pCvarTimer = create_cvar("ironman_timer", "0.1")
	pCvarThrust = create_cvar("ironman_thrust", "125")
	pCvarMaxSpeed = create_cvar("ironman_maxspeed", "400")
	pCvarXyMult = create_cvar("ironman_xymult", "1.05")
	pCvarFuelCost = create_cvar("ironman_fuelcost", "1")
...
public ironman_loop(id)
{
		...
		case true: {
			...
			if ( velocity[0] > gMaxSpeed )		
				velocity[0] = gMaxSpeed
			else if ( velocity[0] < (gMaxSpeed * -1.0) )	
				velocity[0] = gMaxSpeed * -1.0
			if ( velocity[1] > gMaxSpeed )		
				velocity[1] = gMaxSpeed
			else if ( velocity[1] < (gMaxSpeed * -1.0) )	
				velocity[1] = gMaxSpeed * -1.0
			if ( velocity[2] > gThrust * 2.0 )	
				velocity[2] = gThrust * 2.0
