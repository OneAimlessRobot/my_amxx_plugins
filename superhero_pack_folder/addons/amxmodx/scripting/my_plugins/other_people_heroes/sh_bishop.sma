...
public plugin_init()
{
	...
	create_cvar("bishop_level", "7")
	create_cvar("bishop_absorbmult", "0.50")
	create_cvar("bishop_damagemult", "0.75")
	create_cvar("bishop_blastmult", "2.50")
...
public bishop_damage(id)
{
	...
	if ( sh_get_user_has_hero(attacker,gHeroID) && is_user_alive(id) && attacker != id && g_absorbedDamage[attacker] > 0 ) {
		...
		if ( energyDamage > 0 ){
			sh_extra_damage(id, attacker, energyDamage, my_hitpoint_enum:bodypart)
		}
	...
	for (new vic = 1; vic <= SH_MAXSLOTS; vic++) {
		if ( is_user_alive(vic) && ( get_user_team(id) != get_user_team(vic) || FFOn ) ) {
			...
			if ( get_distance(userAim, vicOrigin) <= 150 ) {
				sh_extra_damage(vic, id, blastDamage)
				hit[id] = true
			}
		}
	}
