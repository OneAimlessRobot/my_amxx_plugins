...
public plugin_init()
{
	...
	new pcvarLevel = create_cvar("secretary_level", "0")
	gPcvarRadius = create_cvar("secretary_radius", "2000")
	gPcvarMaxDamage = create_cvar("secretary_maxdamage", "1000")
	gPcvarCooldown = create_cvar("secretary_cooldown", "60")
	gPcvarDelay = create_cvar("secretary_delay", "35")
	gPcvarBurnDecals = create_cvar("secretary_burndecals", "1")
	gPcvarWorkInside = create_cvar("secretary_workinside", "0")
	...
	for ( new i = 0; i < playerCount; i++ ) {
		...
		if ( distanceBetween <= dmgRadius ) {
			if ( FFOn || player == id || idTeam != cs_get_user_team(player) ) {
				...
				sh_extra_damage(player, id, damage, _, SH_DMG_NORM, SH_EXTRA_DMG_FLAG_STUN, ion_mid_origin[id])
			}
		}
