...
public plugin_init()
{
	...
	new pCvarLevel = create_cvar("breaker_level", "12")
	pCvarChance = create_cvar("breaker_chance", "0.25")
	pCvarDeduct = create_cvar("breaker_deduct", "100")
...
{ 
	...
	if (sh_get_user_has_hero(attacker,gHeroID) && generate_float(0.01, 1.00) <= get_pcvar_float(pCvarChance)) 
	{ 	
		new slot=0;
		if(wpnindex < CSW_P90){
