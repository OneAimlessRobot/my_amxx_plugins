...
public plugin_init()
{
	...
	new pcvarLevel = create_cvar("demoman_level", "6")
	gPcvarRadius = create_cvar("demoman_radius", "300")
	gPcvarMaxDamage = create_cvar("demoman_maxdamage", "125")
	gPcvarMaxMines = create_cvar("demoman_maxmines", "2")
	gPcvarMineHealth = create_cvar("demoman_minehealth", "80")
	...
	for ( new i = 0; i < playerCount; i++ ) {
		...
		if ( distanceBetween <= dmgRadius ) {
			if ( FFOn || player == id || idTeam != cs_get_user_team(player) ) {
				...
				sh_extra_damage(player, id, damage, _, SH_DMG_NORM, SH_EXTRA_DMG_FLAG_STUN, vExplodeAt)
			}
