...
new generic_weapon_id = -1
new generic_dmg_source_name[SAFE_BUFFER_SIZE+1]
new generic_dmg_source_log_name[SAFE_BUFFER_SIZE+1]
...
new gBotsEarnXP,gBotsMinLevel,gBotsMaxLevel
...
#if SAVE_METHOD == 1
	#include "my_include/superherovault.inc"	//Saves XP to vault.ini
#endif

#if SAVE_METHOD == 2
	#include "my_include/superheronvault.inc"	//Saves XP to superhero nVault (default)
#endif
...
{	
	...
	server_print("We currently support: %d client states in total!^n",sh_max_client_states())
	...
	sv_superheros = create_cvar("sv_superheros", "1")
	sh_adminaccess = create_cvar("sh_adminaccess", "m")
	sh_alivedrop = create_cvar("sh_alivedrop", "0")
	sh_default_explode_knock_force_magnitude_cvar = create_cvar("sh_default_explode_knock_force_magnitude","3.0")
	sh_default_explode_owner_ignore_frac_cvar = create_cvar("sh_default_explode_owner_ignore_frac","0.5")
	sh_default_stun_speed_pcvar = create_cvar("sh_default_stun_speed","0.5")
	sh_autobalance = create_cvar("sh_autobalance", "0")
	sh_objectivexp = create_cvar("sh_objectivexp", "8")
	sh_cmdprojector = create_cvar("sh_cmdprojector", "1")
	sh_endroundsave = create_cvar("sh_endroundsave", "1")
	sh_hsmult = create_cvar("sh_hsmult", "1.0")
	sh_maxcores = create_cvar("sh_maxcores", "3")
	sh_botsearnxp = create_cvar("sh_botsearnxp", "1")
	sh_botsminlevel = create_cvar("sh_botsminlevel", "5")
	sh_botsmaxlevel = create_cvar("sh_botsmaxlevel", "10")
	sh_loadimmediate = create_cvar("sh_loadimmediate", "0")
	sh_lvllimit = create_cvar("sh_lvllimit", "1")
	sh_maxbinds = create_cvar("sh_maxbinds", "3")
	sh_enable_custom_weapon_logging = create_cvar("sh_enable_custom_weapon_logging", "0")
	sh_maxpowers = create_cvar("sh_maxpowers", "20")
	sh_menumode = create_cvar("sh_menumode", "1")
	sh_mercyxp = create_cvar("sh_mercyxp", "0")
	sh_mercyxpmode = create_cvar("sh_mercyxpmode", "1")
	sh_minlevel = create_cvar("sh_minlevel", "0")
	sh_savexp = create_cvar("sh_savexp", "1")
	sh_saveby = create_cvar("sh_saveby", "1")
	sh_xpsavedays = create_cvar("sh_xpsavedays", "14")
	sh_reloadmode = create_cvar("sh_reloadmode", "1")
	sh_minplrsbhxp = create_cvar("sh_minplayersxp", "2")
	sh_blockvip = create_cvar("sh_blockvip", "abcdef")
	sh_ffa = create_cvar("sh_ffa", "0")
	sh_anubisdmg_check = create_cvar("anubis_showdamage", "1")
...
public plugin_precache()
{
	...
	sh_debug_messages = create_cvar("sh_debug_messages", "0")
}
//----------------------------------------------------------------------------------------------
...
public plugin_cfg()
{
	...
	generic_weapon_id = sh_get_generic_dmg_source_wpn_id()

	xmod_get_wpnname(generic_weapon_id,generic_dmg_source_name,SAFE_BUFFER_SIZE)
	xmod_get_wpnlogname(generic_weapon_id,generic_dmg_source_log_name,SAFE_BUFFER_SIZE)
}
//----------------------------------------------------------------------------------------------
setupConfig()
{
...
localAddXP(id, xp)
{
	...
	if ( xp > 0 && newTotal < playerXP ) {
		...
		if(is_user_bot(id)){
			if(gPlayerLevel[id]>gBotsMaxLevel){
			
				gPlayerLevel[id]=sh_set_user_lvl(id,gBotsMaxLevel)
				return;
			}
			else if(gPlayerLevel[id]<gBotsMinLevel){
			
				gPlayerLevel[id]=sh_set_user_lvl(id,gBotsMinLevel)
				return;
			}
...
public _sh_extra_damage()
{
	...
	new dmg_type=get_param(8)
	new thrashbrat_dmg_type=get_param(9)
	new custom_wpn_id=get_param(10)

	custom_wpn_id = (is_valid_custom_dmg_source(custom_wpn_id))?custom_wpn_id:generic_weapon_id
	...
	else 
	{
		...
		logHit(attacker, victim, wpnDescription,damage_after, floatround(flArmor),bodypart,custom_wpn_id)
	}
}
...
dropPower(id, const said[], showmenu,bool:from_plugin=false)
{
	...
	debugMsg(id, 2, "Trying to Drop Hero: %s", heroName)
...
public bomb_defused(defuser)
{
	...
	if ( !get_pcvar_num(sv_superheros) || !gObjectiveXP ) return PLUGIN_CONTINUE
	if ( !is_user_connected(defuser) ) return PLUGIN_CONTINUE
