...
#define PLUGIN "[CSO] M1887 Maverick"
#define VERSION "1.0"
#define AUTHOR "Dias Pendragon"

#define DAMAGE 62 // 122 for Zombie
...
#define MODEL_V "models/v_m1887craft.mdl"
#define MODEL_P "models/p_m1887craft.mdl"
#define MODEL_W "models/w_m1887craft.mdl"

new const WeaponSounds[6][] = 
{
	"weapons/m1887craft-1.wav",
	"weapons/m1887craft-2.wav",
	"weapons/m1887craft_after_reload.wav",
	"weapons/m1887craft_draw.wav",
	"weapons/m1887craft_insert.wav",
	"weapons/m1887craft_start_reload.wav"
}
...
#define CSW_M1887C CSW_DEAGLE
#define weapon_m1887c "weapon_deagle"

#define WEAPON_SECRETCODE 24062015
...
// Main Vars
new g_Had_M1887C, g_Old_Weapon[33], Float:g_ShootDelay[33], Float:g_Recoil[33]
...
public plugin_init() 
{
	...
	RegisterHam(Ham_Item_Deploy, weapon_m1887c, "fw_Item_Deploy_Post", 1)	
	RegisterHam(Ham_Item_AddToPlayer, weapon_m1887c, "fw_Item_AddToPlayer_Post", 1)
	RegisterHam(Ham_Weapon_Reload, weapon_m1887c, "fw_Weapon_Reload")
	RegisterHam(Ham_Item_PostFrame, weapon_m1887c, "fw_Item_PostFrame")
	RegisterHam(Ham_Weapon_PrimaryAttack, weapon_m1887c, "fw_Weapon_PrimaryAttack")
	RegisterHam(Ham_Weapon_PrimaryAttack, weapon_m1887c, "fw_Weapon_PrimaryAttack_Post", 1)
	//RegisterHam(Ham_Weapon_Reload, weapon_railcannon, "fw_Weapon_Reload_Post", 1)
	
	g_MsgCurWeapon = get_user_msgid("CurWeapon")
	
	register_clcmd("say /m1887", "Get_M1887C")
}
...
public Get_M1887C(id)
{
	Remove_M1887C(id)
	
	Set_BitVar(g_Had_M1887C, id)
	
	give_item(id, weapon_m1887c)
	cs_set_user_bpammo(id, CSW_M1887C, BPAMMO)
	
	static Ent; Ent = fm_get_user_weapon_entity(id, CSW_M1887C)
	if(pev_valid(Ent)) cs_set_weapon_ammo(Ent, CLIP)
	
	engfunc(EngFunc_MessageBegin, MSG_ONE_UNRELIABLE, g_MsgCurWeapon, {0, 0, 0}, id)
	write_byte(1)
	write_byte(CSW_M1887C)
	write_byte(CLIP)
	message_end()
}

public Remove_M1887C(id)
{
	UnSet_BitVar(g_Had_M1887C, id)
}
...
public Event_CurWeapon(id)
{
	...
	if((CSWID == CSW_M1887C && g_Old_Weapon[id] == CSW_M1887C) && Get_BitVar(g_Had_M1887C, id)) 
	{
		static Ent; Ent = fm_get_user_weapon_entity(id, CSW_M1887C)
		if(!pev_valid(Ent))
		{
...
public fw_SetModel(entity, model[])
{
	...
	{
		static weapon
		weapon = fm_get_user_weapon_entity(entity, CSW_M1887C)
		
		if(!pev_valid(weapon))
			return FMRES_IGNORED
		
		if(Get_BitVar(g_Had_M1887C, id))
		{
...
public fw_CmdStart(id, uc_handle, seed)
{
	...
	if(PressButton & IN_ATTACK)
	{
		...
		if(get_gametime() - SPEED > g_ShootDelay[id])
		{
	...
	if((PressButton & IN_RELOAD) && cs_get_weapon_ammo(iEnt) < CLIP && cs_get_user_bpammo(id, CSW_M1887C) > 0 && !get_pdata_int(iEnt, m_fInSpecialReload, XTRA_OFS_WEAPON))
	{
...
public fw_UpdateClientData_Post(id, sendweapons, cd_handle)
{
	if(!is_alive(id))
		return FMRES_IGNORED	
	if(get_player_weapon(id) == CSW_M1887C && Get_BitVar(g_Had_M1887C, id))
...
public fw_PlaybackEvent(flags, invoker, eventid, Float:delay, Float:origin[3], Float:angles[3], Float:fparam1, Float:fparam2, iParam1, iParam2, bParam1, bParam2)
{
	if (!is_connected(invoker))
		return FMRES_IGNORED		
	if(get_player_weapon(invoker) == CSW_M1887C && Get_BitVar(g_Had_M1887C, invoker) && eventid == g_Event_MS)
	{
		...
		static Ent; Ent = fm_get_user_weapon_entity(invoker, CSW_M1887C)
...
public fw_TraceAttack(Ent, Attacker, Float:Damage, Float:Dir[3], ptr, DamageType)
{
	if(!is_connected(Attacker))
		return HAM_IGNORED	
	if(get_player_weapon(Attacker) != CSW_M1887C || !Get_BitVar(g_Had_M1887C, Attacker))
		return HAM_IGNORED
...
public fw_Weapon_Reload(iEnt)
{
	...
	if(!Get_BitVar(g_Had_M1887C, id))
		return HAM_IGNORED	
	if(get_pdata_int(iEnt, m_fInSpecialReload, XTRA_OFS_WEAPON))
...
public fw_Item_PostFrame( iEnt )
{
	...
	switch(get_pdata_int(iEnt, m_fInSpecialReload, XTRA_OFS_WEAPON) )
	{
		case 1: // Check, Start
		{
			if(cs_get_weapon_ammo(iEnt) >= CLIP || cs_get_user_bpammo(id, CSW_M1887C) <= 0)
			{
		...
		case 2: // Insert 
		{
			...
			set_pdata_float(iEnt, m_flTimeWeaponIdle, 0.25, XTRA_OFS_WEAPON)
			set_pdata_float(iEnt, m_flNextPrimaryAttack, 0.25, XTRA_OFS_WEAPON)
			set_pdata_float(iEnt, m_flNextSecondaryAttack, 0.25, XTRA_OFS_WEAPON)
		...
		case 3: // Done Insert
		{
			...
			cs_set_user_bpammo(id, CSW_M1887C, cs_get_user_bpammo(id, CSW_M1887C) - 1)
