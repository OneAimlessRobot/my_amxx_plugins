...
{	
	...
	DatabaseInfoTuple = SQL_MakeDbTuple("","","",DatabaseName)
}

databaseLayoutCreated()
{
	new Handle:connection = getDatabaseConnection()
	
	new bool:exists = sqlite_TableExists(connection,TableName)
...
createDatabaseLayout()
{
	...
	new Handle:query = SQL_PrepareQuery(connection,TableCreateQuery,TableName)
...
Handle:getDatabaseConnection()
{
	static errorCode
	static Handle:connection

	connection = SQL_Connect(DatabaseInfoTuple,errorCode,DatabaseError,charsmax(DatabaseError))
