/* AMX Mod X script.
*
*   Laser Guns (amx_ejl_laserweapon.sma)
...
****************************************************************************
*
*  Add a laser weapon to any Half-Life mod. Fire a laser beam that acts like a
*  real weapon. Takes different damage depending what body zone it hits. All
*  mods can use the laser, but if you run Counter-Strike, the plugin keeps
*  track of all laser related stats in a statsme style. Laser stats restart on
*  the first day of every month. There is FX like smoke clouds, the laserbeams
*  themselves, explosions, laser burns where laserbeam strikes, etc. (burn
*  decals may be an issue with latest hlds, if so, you can turn burn decals
*  with a cvar). If you run the CS missile plugin, the lasers can shoot down
*  the missiles. Also, stats can be tracked by AUTHID or IP address meaning
*  now lan CS servers can take advantage of the stats. Also lasers are
*  sensitive to the mp_friendlyfire cvar. If friendly fire is on, lasers can
*  hurt your teammates, and if off, team- mates can be shot without damage.
*  This plugin has built in TK punish features that can be set to light
*  punishments, or severe.
*
...
*  Additional info:
*
*  Laser kills and damage are sent to the sever logs in the same format as
*  normal kills and damage. A log parser (like pysychostats) should be able to
*  take the log output of this plugin and include them in the stats output.
*  Laser kills count on the CS scoreboard, but are only refreshed at the end
*  of each round. This is for CS only by the way. Plugin may cause some lag at
*  round end depending on the size and power of the server, but it never does
*  anything to cause lag during game play. If its a problem, turn off the
*  amx_laserstats_on cvar (set to 0).
*
...
*	- Added stats_logging.amx style output if stats_loggin.amx is running,
*		can also be enabled with a CVAR for other mods besides CS.
...
*	- Added code so the laser buying can only be enabled for CS
...
new stats_cs_only[] = "[AMXX] Laser Stats are only for Counter-Strike at this point"
new overheat_death[] = "[AMXX] You died because your Laser Gun overheated and exploded"
new overheat_warn[] = "WARNING: Laser Overheating!"  //Prints in center of screen, no [AMXX] needed
new laser_dead[] = "[AMXX] No more power, your laser is dead"
new laser_low[] = "[AMXX] Warning: laser power level low, 10 shots left"
new laser_kill1[] = "[AMXX] You just got fried by %s's frickin Laser Gun." //%s = killers name
new laser_kill2[] = "[AMXX] You just killed %s with a frickin Laser Gun." //%s = victims name
new laser_on[] = "[AMXX] Laser Guns have been enabled by an admin - say /laser for help"
new laser_ona[] = "[AMXX] Laser Guns Enabled" //Message displayed to admin in console
new laser_off[] = "[AMXX] Laser Guns have been diabled by an admin"
new laser_offa[] = "[AMXX] Laser Guns Disabled" //Message displayed to admin in console
new laserbuy_cs[] = "[AMXX] Laser buying is only for Counter-Strike"
new laserbuy_on[] = "[AMXX] Admin has enabled laser buying, each shot costs money"
new laserbuy_ona[] = "[AMXX] Laser Buying Enabled"
new laserbuy_off[] = "[AMXX] Admin has disabed laser buying, laser shots are free"
new laserbuy_offa[] = "[AMXX] Laser Buying Disabled"
new laser_on2[] = "[AMXX] Lasers are ON - For help say /laser" //Printed when somoen says "laser" and stats are off
new laser_on3[] = "[AMXX] Lasers are ON - For help say /laser - Stats say /laserstats" //Used when stats are on
new laser_off2[] =  "[AMXX] Lasers are OFF"
new no_stats[] = "[AMXX] Laser Stats are diasbled"
...
new err_money[] = "[AMXX] Laser shots cost $%d each" // %d = money cost
#endif
...
public plugin_init() {
	register_plugin("Laser Guns","2.7.6","EJL/JTP10181")
