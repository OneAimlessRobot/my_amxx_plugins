...
// Editier-Funktionen
new setupunits = 10	// �nderungen an der Gr��e um diese Einheiten
...
new spr_dot		// ben�tigt f�r die Lininen

new editor = 0		// dieser Spieler ist gerade am erstellen .. Men� verkraftet nur einen Editor
...
public plugin_init() {
	...
	pcv_botdamage = register_cvar("wg_botdamage", "0")	// Bot's sind einfach nur dumm ... und k�nnen nicht lesen
	pcv_immunity = register_cvar("wg_immunity", "0")		// Admins mit Immunit�t brauchen nicht
	pcv_direction = register_cvar("wg_direction", "1")	// zuf�llige Richtung beim Slap
	pcv_botdirection = register_cvar("wg_botdirection", "1") // dito f�r Bots
...
public client_disconnected(player) {
	// aus irgend welchen Gr�nden ist der Spieler einfach wech........
...
public ZoneModeCamper(player) {
	...
	{
		// so ein Feigling ... hat sich einfach verdr�ckt ^^
...
public LoadWGZ() {
	...
	while( (line = read_file(zonefile , line , input , 127 , len) ) != 0 ) 
	{
		...
		new Float:mins[3], Float:maxs[3], Float:pos[3]	// Gr��e & Position
		...
		{
			// �nderungen von CS:CZ zu allen Mods
...
public ShowZoneBox(entity) {
	...
	// m�ssen nicht gezeichnet werden
	...
	// Distanz zum Treffer ... ist Wert gr��er dann war da etwas
...
public HideAllZones() {
	editor = 0	// Men� f�r den n�chsten wieder frei geben ... ufnktionalit�t aktivieren
	for(new i = 0; i < maxzones; i++)
	{
...
public VonRotAbziehen() {
	...
	// k�nnte Probleme geben -> zu klein
