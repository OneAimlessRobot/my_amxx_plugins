...
#endif
new const VERSION[] = "0.82"
new const AUTHOR[] = "DevconeS"

// Define modname
#if defined CONDITION_ZERO
new const MOD_NAME[] = "czero"
#else
new const MOD_NAME[] = "cstrike"
#endif
...
new const SMALL_BLOOD_DECALS[] = {189, 190, 191, 192, 193, 194, 196, 197}	// Small blood decals
#endif
...
public plugin_init()
{
	...
	// Make sure this GameInfo is running on the correct mod
	if(!wpn_set_supported_mod(MOD_NAME, "GameInfo")) return PLUGIN_CONTINUE
	...
	// CVARs
	g_PlayerKillMoney = register_cvar("wpn_playerkill_money", "305")
...
{
	// Well, we wanna make sure this GameInfo is only loaded in the correct mod
	if(!wpn_set_supported_mod(MOD_NAME, _, false))
	{
		// We can't fail here since HLDS would crash if we'd do so
		return PLUGIN_CONTINUE
	}
...
public update_speed()
{
	...
	for(new id = 1; id <= g_MaxPlayers; id++)
	{
		...
		if(usrwpn != -1 && get_user_weapon(id) == WEAPON_INDEX)
		{
...
{
	set_task(4.0, "enable_freeze", TASK_ENABLE_FREEZE)
}
...
public wpn_gi_update_ammo(id, wpnid, pAmmo, sAmmo)
{
	...
	// We send an emessage here because some plugins hook the knife too (e.g. modifying knife speed)
	emessage_begin(MSG_ONE, g_msgCurWeapon, {0, 0, 0}, id)
	ewrite_byte(-1)
	ewrite_byte(29)
	ewrite_byte(-1)
	emessage_end()
