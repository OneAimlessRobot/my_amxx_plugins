...
{
	// PLUGIN INFO
	register_plugin("SUPERHERO Danimoth X","2.1","1sh0t2killz")
...
public sh_hero_init(id, heroID, mode)
{
	...
	// IVISIBILITY LOOP
	if(is_user_alive(id)&&gHasDanimoth[id]){
		set_task(1.0, "Danimoth_loop", id+TASKID, _, _, "b")
	}
		...
		for ( new i = 0; i < playerCount; i++ ) {
			...
			if ( idTeam != cs_get_user_team(player) ) {
				...
				if ( (distanceBetween < DanimothRadius) ) {
					...
					set_task(1.0, "DanimothStasisField",player)
...
//----------------------------------------------------------------------------------------------
public Danimoth_loop(id)
{
	id-=TASKID
	if (!shModActive()) return
	
	if ( gHasDanimoth[id] && is_user_alive(id)&&is_user_connected(id) ){	
			//Cloak Task
		set_task(0.1,"DanimothCloak",id+TASKID)
		set_task(0.1,"DanimothDeCloak",id+TASKID)
	}
}
//----------------------------------------------------------------------------------------------
