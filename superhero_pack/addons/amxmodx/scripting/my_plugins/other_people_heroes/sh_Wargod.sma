...
public Wargod_damage(id)
{
	...
	if ( gHasWargodPower[attacker] && weapon == CSW_DEAGLE && gBullets[attacker] >= 0 && is_user_alive(id) )
	{
		...
		else
		{
			...
			if (!shModActive() || !gHasWargodPower[id] || gPlayerUltimateUsed[id] || !is_user_alive(id)) return PLUGIN_CONTINUE
...
public Wargod_loop()
{
		...
		for ( new id = 1; id <= SH_MAXSLOTS; id++ ) {
				...
				if( gHasWargodPower[id] && is_user_alive(id) ) {
					...
					new players[SH_MAXSLOTS], count, id
