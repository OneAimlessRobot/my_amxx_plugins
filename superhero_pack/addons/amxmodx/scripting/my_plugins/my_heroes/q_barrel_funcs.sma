...
public fw_CmdStart(id, uc_handle, seed)
{
	if(!is_user_alive(id))
		return
	new ammo,clip;
	if(get_user_weapon(id,ammo,clip) != CSW_QUADBARREL || !Get_BitVar(g_Had_QB, id))
		return
...
public fw_PlaybackEvent(flags, invoker, eventid, Float:delay, Float:origin[3], Float:angles[3], Float:fparam1, Float:fparam2, iParam1, iParam2, bParam1, bParam2)
{
	if (!is_user_connected(invoker))
		return FMRES_IGNORED
	new ammo,clip;
	if(get_user_weapon(invoker,ammo,clip) == CSW_QUADBARREL && Get_BitVar(g_Had_QB, invoker) && eventid == g_Event_QB)
	{
...
public fw_TraceAttack(Ent, Attacker, Float:Damage, Float:Dir[3], ptr, DamageType)
{
	if(!is_user_connected(Attacker))
		return HAM_IGNORED	
	new ammo,clip;
	if(get_user_weapon(Attacker,ammo,clip) != CSW_QUADBARREL || !Get_BitVar(g_Had_QB, Attacker))
		return HAM_IGNORED
