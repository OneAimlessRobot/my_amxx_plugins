...
stock const default_file_name[128]="shero_no_help_found.txt"
...
stock superhero_help_files[SH_MAXHEROS][3][STRLEN_FOR_FILES+1];
...
setup_configs(){
	...
	for(new i=0;i<sizeof superhero_help_files;i++){
		...
		arrayset(superhero_help_files[i][FILE_NAME],0,STRLEN_FOR_FILES+1)
	...
	add(default_file, charsmax(default_file), default_file_name, charsmax(default_file_name))

	for(new i=0;i<SH_MAXHEROS;i++){
		
		add(superhero_help_files[i][FILE_NAME],STRLEN_FOR_FILES,default_file, charsmax(default_file))
		add(superhero_help_files[i][TITLE],STRLEN_FOR_FILES,default_title, charsmax(default_title))
		add(superhero_help_files[i][HERO_NAME],STRLEN_FOR_NAMES,default_hero_name, charsmax(default_hero_name))
	}
...
public _superheromod_help_show_hero_help(iPlugins,iParams){
	...
	show_motd(id, superhero_help_files[hero_id][FILE_NAME],superhero_help_files[hero_id][TITLE])


}
public _superheromod_help_link_hero(iPlugins,iParams){
	new hero_id=get_param(1);
	new file_name[STRLEN_FOR_FILES+1]
	new title[STRLEN_FOR_FILES+1]
	new hero_name[STRLEN_FOR_NAMES+1]
	if((hero_id<0)||(hero_id>=SH_MAXHEROS)){
		return
	}
	arrayset(superhero_help_files[hero_id][FILE_NAME],0,STRLEN_FOR_FILES)
	...
	get_array(3, file_name, STRLEN_FOR_FILES);
	get_array(4, hero_name, STRLEN_FOR_NAMES);
	server_print("Trying to register help file of hero named %s!!!!^n",hero_name)
	add(superhero_help_files[hero_id][FILE_NAME],STRLEN_FOR_FILES,help_files_directory, charsmax(help_files_directory))
	add(superhero_help_files[hero_id][FILE_NAME],STRLEN_FOR_FILES,file_name, charsmax(file_name))
	...
	if(!file_exists(superhero_help_files[hero_id][FILE_NAME])){
	
		fclose(fopen(superhero_help_files[hero_id][FILE_NAME], "a+"));
		if(!file_exists(superhero_help_files[hero_id][FILE_NAME])){
			log_error(AMX_ERR_NOTFOUND,"File of path %s could not be created^nEven when it doesnt exist!!!^n",
											superhero_help_files[hero_id][FILE_NAME]) 
			arrayset(superhero_help_files[hero_id][FILE_NAME],0,STRLEN_FOR_FILES)
			add(superhero_help_files[hero_id][FILE_NAME],STRLEN_FOR_FILES,default_file, charsmax(default_file))
...
stock print_help_file(id,hero_index){
		...
		if(is_user_connected(id)){
			...
			console_print(id,"[SH] (Hero help files): Hero help file path: %s",
						superhero_help_files[hero_index][FILE_NAME])
...
stock server_print_help_file(hero_index){
		...
		server_print("[SH] (Hero help files): Hero help title: %s",
					superhero_help_files[hero_index][TITLE])
		server_print("[SH] (Hero help files): Hero help file path: %s",
					superhero_help_files[hero_index][FILE_NAME])
