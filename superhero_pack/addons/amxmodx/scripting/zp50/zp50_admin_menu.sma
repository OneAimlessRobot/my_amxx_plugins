...
{
	if (equal(module, LIBRARY_NEMESIS) || equal(module, LIBRARY_ASSASSIN) || equal(module, LIBRARY_DRAGON) || equal(module, LIBRARY_SURVIVOR) || equal(module, LIBRARY_SNIPER) || equal(module, LIBRARY_KNIFER))
...
show_menu_admin(id)
{
	...
	if (LibraryExists(LIBRARY_DRAGON, LibType_Library) && (userflags & read_flags(g_access_make_dragon)))
		len += formatex(menu[len], charsmax(menu) - len, "\r4.\w Make Dragon^n", id)
	else
		len += formatex(menu[len], charsmax(menu) - len, "\d4. Make Dragon^n", id)
	...
	if (userflags & read_flags(g_access_respawn_players))
		len += formatex(menu[len], charsmax(menu) - len, "\r8.\w %L^n", id, "MENU_ADMIN6")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\d8. %L^n", id, "MENU_ADMIN6")
	
	// 9. Start Game Mode command
	if (userflags & read_flags(g_access_start_game_mode))
		len += formatex(menu[len], charsmax(menu) - len, "\r9.\w %L^n", id, "MENU_ADMIN_START_GAME_MODE")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\d9. %L^n", id, "MENU_ADMIN_START_GAME_MODE")
...
public menu_player_list(id, menuid, item)
{
	...
	if (is_user_connected(player))
	{
		...
		switch (PL_ACTION)
		{
			...
			case ACTION_MAKE_KNIFER: // Sniper command
			{
				if (LibraryExists(LIBRARY_KNIFER, LibType_Library) && (userflags & read_flags(g_access_make_knifer)) && is_user_alive(player) && !zp_class_knifer_get(player))
