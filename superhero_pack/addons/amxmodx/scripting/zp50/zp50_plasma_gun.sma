...
public primary_attack(id)
{
	...
	if(is_user_alive(iTarget) && !zp_class_nemesis_get(iTarget) && !zp_item_zombie_madness_get(iTarget) && !zp_class_assassin_get(iTarget) && !zp_class_dragon_get(iTarget))
	{	
		if(HITSD > 0.0)
		{
		...
		if(zp_core_is_zombie(iTarget) || zp_class_nemesis_get(iTarget))
		{
