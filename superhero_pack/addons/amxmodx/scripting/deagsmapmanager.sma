...
#if FILE_LOGLEVEL >= LOGLEVEL_DEBUG
			log_to_file(logfilename, "[DMM] DEBUG: Vote (name:map): %s:%s;", name, nmaps[nnextmaps[key]]);
#endif
			c_p(0, print_chat, "[DMM] * %L", LANG_PLAYER, "DMAP_CHOSE_MAP", name, nmaps[nnextmaps[key]]);
		}
	}
	nvotes[key] += 1;
	g_TotalVotes += 1;
...
show_vote_menu(bool:bFirstTime) {
	...
	for (nmapstoch = 0; nmapstoch < dmax; ++nmapstoch) {
		...
		if (is_custom_map(nmaps[nnextmaps[nmapstoch]]) && usestandard) {
			if (bIsCstrike) {
				formatex(sMenuOption, charsmax(sMenuOption), "%d. %s  \b(%L)\w", nmapstoch + 1, nmaps[nnextmaps[nmapstoch]], LANG_SERVER, "DMAP_MENU_CUSTOM");
			} else {
				formatex(sMenuOption, charsmax(sMenuOption), "%d. %s  (%L)", nmapstoch + 1, nmaps[nnextmaps[nmapstoch]], LANG_SERVER, "DMAP_MENU_CUSTOM");
			}
		} else {	// Don't show (Custom)
			formatex(sMenuOption, charsmax(sMenuOption), "%d. %s", nmapstoch + 1, nmaps[nnextmaps[nmapstoch]]);
		}
	...
	if (!mp_winlimit && get_cvar_float("mp_timelimit") < get_pcvar_float(pExtendmapMax)) {
		formatex(sMenuOption, charsmax(sMenuOption), "%L^n", LANG_SERVER, "DMAP_MENU_EXTEND", maps_to_select + 1, mapname, steptime);
	...
	formatex(sMenuOption, charsmax(sMenuOption), "%L", LANG_SERVER, "DMAP_MENU_NONE", maps_to_select + 2);
