...
#define V_MODEL "models/ByM_Cod/v_bow.mdl"
#define P_MODEL "models/ByM_Cod/p_bow.mdl"
#define P_MODEL_EMPTY "models/ByM_Cod/p_bow_empty.mdl"
#define W_MODEL "models/ByM_Cod/w_bow.mdl"
#define ARROW_MODEl "models/ByM_Cod/arrow.mdl"
...
new const WeaponResources[3][] = {
	"sprites/ByM_Cod/weapon_bow.txt",
	"sprites/ByM_Cod/640hud12_2.spr",
	"sprites/ByM_Cod/640hud98_2.spr"
}
...
#define CSW_BOW CSW_XM1014
#define weapon_bow "weapon_xm1014"
#define BOW_AMMOID 4

#define ARROW_DEFAULT 20
...
#define DAMAGE_A 100
#define DAMAGE_B 9999

#define WEAPON_SECRETCODE 3102013
#define WEAPON_ANIMEXT "carbine"
#define WEAPON_OLDWMODEL "models/w_xm1014.mdl"

#define WL_NAME "ByM_Cod/weapon_bow"
...
new g_SprId_LaserBeam, g_sprExp;
new CsTeams: g_iPlayerTeam[ 33 ];

public plugin_init() {
	register_plugin("CoD Weapon : Compound Bow", "1.0", "Dias & N3u[T]r4L & Milutinke")
	bym_ucitaj_api( );
	
	RegisterHam( Ham_Spawn, "player", "fw_HamSpawnPost", .Post = true );
	...
	RegisterHam( Ham_TakeDamage, "player", "NanesiStetu" );
...
public plugin_precache() {
	...
	g_sprExp = precache_model( "sprites/ByM_Cod/ArrowExplosion.spr" )
	...
	g_SprId_LaserBeam = engfunc( EngFunc_PrecacheModel, "sprites/ByM_Cod/Trail.spr" );
}
...
public fw_HamSpawnPost( iPlayer ) {
	if( !is_user_connected( iPlayer ) || !( 1 <= iPlayer <= get_maxplayers( ) ) )
		return HAM_IGNORED;
		
	g_iPlayerTeam[ iPlayer ] = cs_get_user_team( iPlayer );
	
	return HAM_IGNORED;
}

public NanesiStetu( iVictim, iInflictor, iAttacker, Float: fDamage, iDamageType ) {
	if( !bym_igrac_je( iVictim, Ziv ) || !bym_igrac_je(iVictim, Povezan ) || !bym_igrac_je( iAttacker, Ziv ) || !bym_igrac_je( iAttacker, Povezan ) )
		return HAM_IGNORED;
		
	if( get_user_weapon( iAttacker ) == CSW_BOW ) {
...
public plugin_natives() {
	register_native("cod_give_compound_bow", "GiveBow", 1);
	register_native("cod_get_compound_bow", "GetBow", 1);
}

public GiveBow(Index) {
	Set_BitVar(g_Had_CompoundBow, Index);
	g_BowArrow[Index] = ARROW_DEFAULT;
	g_WeaponState[Index] = WEAPON_NONE;
	
	fm_give_item(Index, weapon_bow);
	UpdateAmmo(Index, CSW_BOW, BOW_AMMOID, -1, g_BowArrow[Index]);
}

public GetBow(Index) Remove_CompoundBow(Index);
...
{
	if(get_user_weapon(id) != CSW_BOW || !Get_BitVar(g_Had_CompoundBow, id) || !bym_igrac_je( id, Ziv ) || g_iPlayerTeam[ id ] == CS_TEAM_SPECTATOR || g_iPlayerTeam[ id ] == CS_TEAM_UNASSIGNED )
		return
