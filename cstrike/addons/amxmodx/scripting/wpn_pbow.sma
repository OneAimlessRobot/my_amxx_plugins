   ...
   /* - - - - - - - - - - -
    ...
    /* - - -
     |  Customization  |
                 - - - */
        #define CROSSBOW_RECOIL            1.0     // Recoil ( float )
        #define CROSSBOW_RUN_SPEED         620.0   // Max speed ( float )
        #define CROSSBOW_AMMO1             100      // Clip size
        #define CROSSBOW_AMMO2             200      // Ammo size
        #define CROSSBOW_COST              175000   // Cost
        
        #define ZOOM1_WANTED               45      // Primary zoom.
        #define ZOOM2_WANTED               30       // Secondaty zoom.
        
        #define BOLT_AIR_VELOCITY          2000    // Speed in air.
        #define BOLT_WATER_VELOCITY        1000    // Speed in water.
        
        #define BOLT_DAMAGE_MIN            99      // Bolt touches you. Min damage.
        #define BOLT_DAMAGE_MAX            100     // Bolt touches you. Max damage.
        #define BOLT_DAMAGE_MONSTER        200     // Bolt damage on monster.
            
        #define BOLT_EXPLOSION_DAMAGE_MAX  35.0    // Bolt explodes. Max damage. ( float )
        #define BOLT_EXPLOSION_RANGE       170.0   // Bolt explodes. Max range.  ( float )
        
        #define BOLT_SNIPER_DAMAGE_MIN     100    // Bolt sniped. Min damage.
        #define BOLT_SNIPER_DAMAGE_MAX     120    // Bolt sniped. Max damage
        #define BOLT_SNIPER_LIFE           10.0   // Delay before removing. ( float )
        
        #define BOLT_SHOW_TRACERS          1      // 0 = Don't show tracers ;  1 = Show tracers.
        
        #define BOLT_TRACER_RED           200     // Red
        #define BOLT_TRACER_GREEN         100     // Green
        #define BOLT_TRACER_BLUE          100     // Blue
        #define BOLT8TRACER_BBRIGHTNESS   255     // Brightness
    
    /* - - -
     |  Weapon information   |
                       - - - */
        new
            gs_WpnName [] = "Black Pirate-Crossbow",
            gs_WpnShort[] = "pbow";
    ...
    /* - - -
     |  Weapon model   |
        ...
        new
            ...
            gs_Model_V[] = "models/wpnmod/v_pbow.mdl",
    ...
    /* - - -
     |    Others stuff   |
        ...
        new const gs_BoltExplodeClassname[] = "wpn_bolt_explode1";
        new const gs_BoltClassname       [] = "wpn_bolt1";
    ...
    public plugin_precache()
    {
        ...
        gi_Fireball   = precache_model ( "sprites/nhth1.spr" );
        
        #if BOLT_SHOW_TRACERS == 1
            gi_Laser      = precache_model ( "sprites/xbeam2.spr" );
            gi_SmokeTrail = precache_model ( "sprites/xbeam5.spr" );
        #endif
    }
    ...
    CreateWeapon ()
    {
        ...
        wpn_set_float ( i_Weapon_id, wpn_refire_rate1, 0.1 );
        wpn_set_float ( i_Weapon_id, wpn_refire_rate2, 0.40 );
        ...
        wpn_set_integer ( i_Weapon_id, wpn_bullets_per_shot2, 0 );
    ...
    {
        wpn_set_float ( gi_Weaponid, wpn_refire_rate2 , 0.40 );
    ...
    public Crossbow_SecondaryAttack( const id )
    {
        ...
        wpn_set_float ( gi_Weaponid, wpn_refire_rate2 , 0.25 );
    }


    public Crossbow_Deploy ( const id )
    {
        wpn_playanim ( id, !IsAmmoEmpty ( id, usr_wpn_ammo1 ) ? draw1 : draw2 );
    }
    ...
    {
        ResetZoom ( id );
        wpn_playanim ( id, !IsAmmoEmpty ( id, usr_wpn_ammo1 ) ? holster1 : holster2 );
    }
    ...
    FireBolt ( const id )
    {
        ...
        wpn_set_float ( gi_Weaponid, wpn_refire_rate1 , 0.1 );
        wpn_set_float ( gi_Weaponid, wpn_refire_rate2 , 0.40 );
    ...
    {
        wpn_playanim ( id, !IsAmmoEmpty ( id, usr_wpn_ammo1 ) ? fire1 : fire3 );
    ...
    WeaponIdle ( const id )
    {
        ...
        if ( f_Rand < 0.75 )
        {
            wpn_playanim ( id, b_HasClip ? idle1 : idle2 );
        ...
        else
        {
            if ( b_HasClip )
            {
                wpn_playanim ( id, fidget1 );
                gf_TimeWeaponIdle[ id ] = get_gametime () + 90.0 / 30.0;
            }
            else
            {
                wpn_playanim ( id, fidget2 );
