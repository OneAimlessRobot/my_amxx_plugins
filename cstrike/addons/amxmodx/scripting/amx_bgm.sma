...
g_FileName[]    = "bgm",     // Name of the main file if no files is found in 'g_FileFolder'.
g_FileFolder[]  = "amx_bgm",    // Name of the directory in amxmodx/configs/ for per-map files.
g_FilePrefix[]  = "ini",            // File extension used for the files.
g_CmdChat[]     = "/bgm";    // Chat command for player.
...
#if defined old_amxx
new g_sBgm_sounds[ MAX_SOUNDS ][ MAX_FILE_LENGTH ];
...
{
register_dictionary( "amx_bgm.txt" );

if( ( p_random_precache = get_cvar_pointer( "bgm_random_precache" ) ) )
...
#if !defined old_amxx
g_lstSound = ArrayCreate( MAX_FILE_LENGTH );
#endif
...
ShowPrint( id, "%L", id, "BGM_DISPLAY_ADS", g_CmdChat );
}
...
{
ShowPrint( id, "%L", id, "BGM_CMD_DISABLED" );
return PLUGIN_HANDLED;
}

g_pHeardSound[id] = g_pHeardSound[id] ? false : true;
ShowPrint( id, "%L", id, g_pHeardSound[id] ? "BGM_SOUND_ENABLED" : "BGM_SOUND_DISABLED" );
...
#if defined old_amxx
copy( sCurSnd, MAX_FILE_LENGTH - 1, g_sBgm_sounds[ iRand ] );
#else
ArrayGetString( g_lstSound, iRand, sCurSnd, MAX_FILE_LENGTH - 1 );
#endif
...
bFound ?
log_amx( "%L", LANG_SERVER, "BGM_LOG_LOADING", sPath ) :
log_amx( "%L", LANG_SERVER, "BGM_LOG_NO_FILES_FOUND" );
...
{
log_amx( "%L", LANG_SERVER, "BGM_LOG_UNKNOW_EXTENSION", sExt );
continue;
}

if( !file_exists( sLeft ) )
{
log_amx( "%L", LANG_SERVER, "BGM_LOG_INEXISTENT_FILE", sLeft );
continue;
}

#if defined old_amxx
copy( g_sBgm_sounds[ g_nSnd ][ 0 ], MAX_FILE_LENGTH - 1, sLeft );
#else
ArrayPushString( g_lstSound, sLeft );
#endif
...
{
new iMax;
GetPrecacheValue( iMax);

#if defined old_amxx
UpdateArray( iMax_t );
#else
p_DeleteRandomItem( iMax , g_lstSound  );
#endif
}

log_amx( "---" );
#if defined _DEBUG
log_amx( "[ Loading %d Sounds ]", g_nSnd);
#endif
#if defined old_amxx
PrecacheSounds(  );
#else
PrecacheSounds_n( g_lstSound );
#endif
}
...
if( pos > 0 )
{
iMax = str_to_num( g_sRp_value[ pos + 1 ] )
}
else
{
iMax  = str_to_num( g_sRp_value );
}
}


stock UpdateArray( iMax )
{
new const iCnt_sound = g_nSnd;
...
static
sTmp_sounds[ MAX_SOUNDS ][ MAX_FILE_LENGTH ],
iLast_number[ MAX_SOUNDS ];

new i, iRand;
for( i = 0; i < iCnt_sound; i++ )
{
copy( sTmp_sounds[i], MAX_FILE_LENGTH - 1, g_sBgm_sounds[i] );
g_sBgm_sounds[i][0] = '^0';
}
...
copy( g_sBgm_sounds[i], MAX_FILE_LENGTH - 1, sTmp_sounds[ iRand ]);
++i;
...
stock p_DeleteRandomItem( iMax, Array:sSound_a )
{
new const iCnt_sound = g_nSnd;
...
{
for( new i; i < g_nSnd; i++ )
{
PrecacheFile( g_sBgm_sounds[i] );

#if defined _DEBUG
log_amx( "   - %s", g_sBgm_sounds[i] );
#endif
}
...
formatex( message, charsmax( message ), "^4[BGM]^1 %s", newMsg );
...
if( ( contain( sBuffer, ".mp3^"" ) != -1 || contain( sBuffer, ".wav^"" ) != -1 ))
return true;
}
fclose( fp );
